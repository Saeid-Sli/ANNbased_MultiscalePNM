...
start_time = time.time()
highResNetwork, microThroats, macroThroats, randomState = connetNetworks.connect(
    network, 
    highResNetworks, 
    neighbors, 
    connectedRegions,
...
np.save(output_path + 'randomState.npy', randomState) 
...
poreNumsAfterTrim = len(highResNetwork.Ps)
throatNumsAfterTrim = len(highResNetwork.Ts)

op.topotools.find_surface_pores(highResNetwork)
op.topotools.label_faces(highResNetwork)

highResNetwork['pore.diameter'] = highResNetwork['pore.equivalent_diameter']
highResNetwork['throat.diameter'] = highResNetwork['throat.equivalent_diameter']
highResNetwork['throat.length'] = highResNetwork['throat.total_length']

highResNetwork.add_model(propname='pore.area',
                  model = op.models.geometry.pore_cross_sectional_area.sphere)
highResNetwork.add_model(propname='throat.area',
                  model = op.models.geometry.throat_cross_sectional_area.cylinder)

highResNetwork.add_model(propname='throat.endpoints',
             model=op.models.geometry.throat_endpoints.spherical_pores,
             pore_diameter='pore.diameter',
             throat_diameter='throat.diameter')


highResNetwork.add_model(propname='throat.conduit_lengths',
                  model = op.models.geometry.throat_length.conduit_lengths)

highResNetwork.add_model(propname='throat.volume',
...
geometry = op.geometry.GenericGeometry(network=highResNetwork,pores=highResNetwork.Ps,throats=highResNetwork.Ts)

for i in highResNetwork.props():
    if i not in ['throat.conns','pore.coords']:
        geometry.update({i:highResNetwork.pop(i)})

phase_inv = op.phases.Air(network = highResNetwork)
phase_def = op.phases.Water(network = highResNetwork)

phys_inv = op.physics.GenericPhysics(network=highResNetwork, phase=phase_inv, geometry=geometry)
phys_def = op.physics.GenericPhysics(network=highResNetwork, phase=phase_def, geometry=geometry)
...
L = [f"Micro throats = {microThroats} \n", f"Macro throats = {macroThroats} \n", 
     f"Pore numbers before ANN = {poreNumsBefore}  \n",  f"throat numbers before ANN = {throatNumsBefore}  \n",\
     f"Pore numbers = {poreNums} \n", f"Throat numbers = {throatNums} \n",\
     f"Pore numbers after trim = {poreNumsAfterTrim} \n", f"Throat numbers after trim = {throatNumsAfterTrim} \n",\
     f"Porosity = {porosity} \n", \
     f"Permeability_X = {K1} \n", f"Permeability_Y = {K2} \n", \
